...
static jintArray RishHost_startHost(
    ...
    bool direct_pty = tty && stdin_read < 0 && stdout_write < 0 && stderr_write < 0;
    ...
    if (pid > 0) {
        ...
        const char *executable = direct_pty ? argv[0] : "/system/bin/sh";
        if (envv) {
            if (execvpe(executable, (char *const *) argv, (char *const *) envv) == -1) {
                PLOGE("execv");
                exit(1);
            }
        } else {
